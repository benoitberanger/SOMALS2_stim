function varargout = gui_SOMALS2
% gui_SOMALS2 is the function that creates (or bring to focus) SOMALS2 GUI.
% Then, main_SOMALS2 is always called to start each task. It is the
% "main" program.
...
if ~isempty(figPtr) % Figure exists so brings it to the focus
    ...
    b_ELEC.x   = p_task.xpos(p_task.count);
    b_ELEC.w   = p_task.xwidth(p_task.count);
    b_ELEC.y   = 0.05;
    b_ELEC.h   = 0.50;
    b_ELEC.tag = 'pushbutton_ELEC';
    handles.(b_ELEC.tag) = uicontrol(handles.uipanel_Task,...
        'Style','pushbutton',...
        'Units', 'Normalized',...
        'Position',[b_ELEC.x b_ELEC.y b_ELEC.w b_ELEC.h],...
        'String','ELEC',...
        'BackgroundColor',buttonBGcolor,...
        'Tag',b_ELEC.tag,...
    ...
    p_task.count  = p_task.count + 1;
    b_PNEU.x   = p_task.xpos(p_task.count);
    b_PNEU.w   = p_task.xwidth(p_task.count);
    b_PNEU.y   = 0.05;
    b_PNEU.h   = 0.50;
