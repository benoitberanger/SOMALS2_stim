...
switch get(hObject,'Tag')
    ...
    otherwise
        error('SOMALS2:TaskSelection','Error in Task selection')
end
...
switch get(get(handles.uipanel_SaveMode,'SelectedObject'),'Tag')
    ...
    case 'radiobutton_NoSave'
        SaveMode = 'NoSave';
    otherwise
        warning('SOMALS2:SaveSelection','Error in SaveMode selection')
end
...
switch get(get(handles.uipanel_OperationMode,'SelectedObject'),'Tag')
    ...
    otherwise
        warning('SOMALS2:ModeSelection','Error in Mode selection')
end
...
switch get(get(handles.uipanel_EyelinkMode,'SelectedObject'),'Tag')
    ...
    case 'radiobutton_EyelinkOn'
        ...
        if isempty(status)
            error('SOMALS2:EyelinkToolbox','no ''Eyelink.m'' detected in the path')
        end
        
        % Save mode ?
        if strcmp(S.SaveMode,'NoSave')
            error('SOMALS2:SaveModeForEyelink',' \n ---> Save mode should be turned on when using Eyelink <--- \n ')
        end
    ...
    otherwise
        ...
        warning('SOMALS2:EyelinkMode','Error in Eyelink mode')
...
switch get(handles.checkbox_WindowedScreen,'Value')
    ...
    case 0
        WindowedMode = 'Off';
    otherwise
        warning('SOMALS2:WindowedScreen','Error in WindowedScreen')
